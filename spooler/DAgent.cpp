...
int main(int argc, char **argv)
{
	...
	if (!loglevel)
		...
		g_lpLogger = CreateLogger(g_lpConfig.get(), argv[0]);
	ec_log_set(g_lpLogger);
	...
	StatsClient(std::move(cfg))
{
	...
	AddStat(SCN_DAGENT_MAX_THREAD_COUNT, SCT_INTEGER, "dagent_thread_maxed", "Number of maximum threads created");
	...
	AddStat(SCN_DAGENT_STRINGTOMAPI, SCT_INTEGER, "dagent_stringtomapi", "Conversions of emails into MAPI messages");
	AddStat(SCN_DAGENT_TO_COMPANY, SCT_INTEGER, "dagent_to_company", "Number of mails delivered to a company");
	...
	AddStat(SCN_LMTP_RECEIVED, SCT_INTEGER, "lmtp_received", "LMTP DATA received");
